#!/usr/bin/env node
// copied from npm nonamekill_server
;(function () {
    var WebSocketServer = require("ws").Server
    var wss = new WebSocketServer({ port: 8080 })
    console.log("服务器开启成功,本地联机地址是127.0.0.1")
    var os = require("os")
    ...
    Object.keys(ifaces).forEach(function (ifname) {
      ifaces[ifname].forEach(function (iface) {
        if (
          ...
          i++
          console.log("局域网联机地址: " + iface.address)
          return
        }
      })
    })
    ...
    if (i == 0) {
      console.log("没有找到局域网联机地址,可能是你没有连接wifi.")
    } else if (i > 1) {
      console.log("联机地址可能是上面其中的一个.")
    }
    var bannedKeys = []
    var bannedIps = []
