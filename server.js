...
console.log("服务器开启成功")
console.log(`\t本地联机地址localhost:${port}或127.0.0.1:${port}`)
...
Object.keys(ifaces).forEach(function (ifname) {
  ifaces[ifname].forEach(function (iface) {
    if (
      ...
      iface.address.substr(0, 8) == "192.168."
    ) {
      i++
      console.log("\t局域网联机地址: " + iface.address + ":" + port)
      return
    }
  })
})
...
if (i == 0) {
  console.log("\t局域网联机地址未找到，可能是你没有连接wifi")
} else if (i > 1) {
  console.log("\t局域网联机地址可能是上面其中的一个")
}
console.log("请用浏览器访问以上地址进入游戏，并在联机大厅填写相同地址")